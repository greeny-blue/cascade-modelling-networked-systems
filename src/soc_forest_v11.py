...
#
# Manual screenshots:
#   - Press 's' during graphics mode to save a PNG with "manual" in the filename.
#
# Automatic screenshots:
#   - Use --auto-screenshots (graphics mode only).
#   - Each CA step during a fire is saved under screenshot_dir (default
#     'socsim_automatic_screenshots') as:
#         fire_00001_step_00010.png
# ------------------------------------------------------------
...
STEPS_PER_FRAME_DEFAULT = 100      # how many DRIVE steps per video frame
...
# IMPORTANT: graphics ON by default now
NO_GRAPHICS_DEFAULT = False

LIMIT_BY_DEFAULT = "fires"          # "fires" or "frames"
TRACK_DENSITY_DEFAULT = True
DENSITY_LOG_INTERVAL_DEFAULT = 1000  # in frames
...
def main():
    ...
    # Warn if auto screenshots requested but no graphics
    if auto_screenshots and no_graphics:
        print("WARNING: --auto-screenshots requested but graphics are disabled (--no-graphics).")
        print("         No screenshots will be saved. Run WITHOUT --no-graphics to capture images.\n")
    ...
    if pre_fire_states:
        arr = np.stack(pre_fire_states, axis=0)  # (n_fires, n_rows, n_cols)
